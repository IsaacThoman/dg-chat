...
Deno.test("deduplicated generated stages keep exactly one keyed cleanup job", () => {
  const { repo, owner, model, identitySnapshot, createAttachment } = fixture();
  const reused = createAttachment("deduplicated-output", "f");
  const stage = repo.stageGeneratedObject({
    ownerId: owner.id,
    usageRunId: "asset-run-0001",
    ordinal: 0,
  ...
  repo.markGeneratedObjectStored(stage.id, owner.id);
  repo.attachGeneratedObject(stage.id, owner.id, reused.id, false);
  repo.finalizeGeneratedAssets({
    ownerId: owner.id,
    usageRunId: "asset-run-0001",
    providerModelId: model.id,
    ...identitySnapshot,
    idempotencyKey: "deduplicated-generated-cleanup",
    requestHash: "9".repeat(64),
    operation: "generation",
    prompt: "Reuse equal generated bytes",
    providerCreatedAt: 1_700_000_001,
    assets: [{
      attachmentId: reused.id,
      ordinal: 0,
      width: 1,
      height: 1,
    }],
  ...
  assertEquals(repo.generatedObjectStages.get(stage.id)?.state, "cleanup_pending");
  assertEquals(
    repo.jobs.filter((job) => job.idempotencyKey === `generated_object.cleanup:${stage.id}`).length,
    1,
  );
  assertEquals(repo.requestGeneratedObjectCleanup(owner.id, "asset-run-0001", "retry"), 1);
  assertEquals(
    repo.jobs.filter((job) => job.idempotencyKey === `generated_object.cleanup:${stage.id}`).length,
    1,
  );
...
Deno.test("generated attachment admission rolls back when its stage cannot be attached", () => {
  const { repo, owner } = fixture();
  const objectKey = `generated/${owner.id}/${crypto.randomUUID()}.png`;
  const stage = repo.stageGeneratedObject({
    ownerId: owner.id,
    usageRunId: "asset-run-0001",
    ordinal: 0,
  ...
  repo.markGeneratedObjectStored(stage.id, owner.id);
  const attachmentsBefore = repo.attachments.size;
  const blobsBefore = repo.attachmentStorageBlobs.size;
  const jobsBefore = repo.jobs.length;
  repo.attachGeneratedObject = (() => {
    throw new DomainError(
      "generated_stage_conflict",
      "Simulated generated-stage attach failure",
      409,
    );
  }) as typeof repo.attachGeneratedObject;
  ...
  assertEquals(repo.attachments.size, attachmentsBefore);
  assertEquals(repo.attachmentStorageBlobs.size, blobsBefore);
