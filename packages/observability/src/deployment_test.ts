...
Deno.test("Compose keeps per-replica exporters private and starts monitoring before health", async () => {
  ...
  assertMatch(
    alerts,
    /alert: DgChatApiTargetsAbsent[\s\S]*?expr: absent\(up\{job="dg-chat-api"\}\)/u,
  );
  assertMatch(
    alerts,
    /alert: DgChatWorkerTargetsAbsent[\s\S]*?expr: absent\(up\{job="dg-chat-worker"\}\)/u,
  );
  ...
  assertMatch(acceptance, /absent\(up\{job="dg-chat-worker"\}\)/u);
