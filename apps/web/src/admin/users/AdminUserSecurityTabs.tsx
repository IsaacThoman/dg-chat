...
export function AdminUserSessionsTab(
  ...
  const revoke = async (reason: string) => {
    ...
    try {
      ...
      if (isConflict(cause)) await sessions.refetch();
      throw cause;
    }
  };
  return (
    <section className="admin-user-tab-stack" aria-labelledby="admin-user-sessions-heading">
      ...
      {sessions.isError && sessions.data && (
        <p className="admin-user-stale-warning" role="status">
          The latest session refresh failed. Showing the last loaded data.{"  "}
          <button type="button" className="text-button" onClick={() => void sessions.refetch()}>
            Retry
          </button>
        </p>
      )}
          ...
          {sessions.data.data.map((session) => (
            <li key={session.id}>
              ...
              <button
                ...
                disabled={session.current || session.status !== "active"}
...
export function AdminUserTokensTab(
  ...
  const revoke = async (reason: string) => {
    ...
    try {
      ...
      announce("API token family revoked.");
    } catch (cause) {
      if (isConflict(cause)) await tokens.refetch();
      throw cause;
    }
  };
  return (
    <section className="admin-user-tab-stack" aria-labelledby="admin-user-tokens-heading">
      ...
      {tokens.isError && tokens.data && (
        <p className="admin-user-stale-warning" role="status">
          The latest token refresh failed. Showing the last loaded data.{"  "}
          <button type="button" className="text-button" onClick={() => void tokens.refetch()}>
            Retry
          </button>
        </p>
      )}
