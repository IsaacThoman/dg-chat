...
export function VoiceRecorder(
  { model, disabled, onTranscript }: {
    model?: string;
    disabled?: boolean;
    onTranscript: (text: string) => void;
  },
) {
  const voice = useVoiceRecorder(model ?? "", onTranscript);
        ...
        : (
          ...
          >
            ...
            <div className="voice-actions">
              {state.phase === "recording" && (
                <button type="button" className="secondary" autoFocus onClick={voice.stop}>
                  <Square size={14} /> Stop
                </button>
              )}
              {(state.phase === "preview" || (state.phase === "error" && state.previewUrl)) && (
                <>
                  ...
                  <button type="button" className="primary" onClick={voice.transcribe}>
                    Insert transcript
                  </button>
                </>
              )}
