...
Deno.test("admin usage returns bounded aggregates without materializing the ledger", async () => {
  const { app, repository, headers } = await adminFixture();
  const user = repository.createUser({
    email: "usage-summary-user@example.test",
    name: "Usage summary user",
    passwordHash: "test",
  });
  repository.credit(user.id, "usage-summary-grant", "grant", 200);
  repository.reserve(user.id, "usage-summary-run", "test/model", 50);
  const expected = repository.adminSummary();
  assertEquals(repository.ledger.length > 0, true);
  Object.defineProperty(repository, "ledger", {
    configurable: true,
    get() {
      throw new Error("admin usage must not materialize the ledger");
    },
  });

  const response = await app.request("/api/admin/usage", { headers });
  assertEquals(response.status, 200, await response.clone().text());
  assertEquals(response.headers.get("cache-control"), "private, no-store");
  const body = await response.json() as Record<string, unknown>;
  assertEquals(body, expected);
  assertEquals("ledger" in body, false);
});
